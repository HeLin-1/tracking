...
#include <arm_neon.h>
void change_format_neon(float* des, float* source, int height, int width, int channel) {
    const int channel_stride = height * width; 
    for (int k = 0; k < channel; ++k) {
        float* dst_ptr = des + k * channel_stride;
        for (int j = 0; j < width; ++j) {
            float* src_ptr = source + j * channel + k;
            // ARM NEON向量化
            #ifdef __ARM_NEON
            // 处理能被4整除的高度
            int i = 0;
            for (; i <= height - 4; i += 4) {
                ...
                float32x4_t v = {
                    ...
                    *(src_ptr + 2 * width * channel),
                    *(src_ptr + 3 * width * channel)
                };
                
                // 存储到连续内存
                vst1q_f32(dst_ptr, v);
                
                src_ptr += 4 * width * channel;
                dst_ptr += 4;
            }
...
cv::Mat fhog(const cv::Mat& input, int binSize, int nOrients, float clip, bool crop){
    ...
    std::vector<float> I(HEIGHT*WIDTH*DEPTH);
    change_format_neon(I.data(),normalized.ptr<float>(),HEIGHT,WIDTH,DEPTH);

    int h,w,d;
    float* HH = fhog(I.data(),HEIGHT,WIDTH,DEPTH,&h,&w,&d,binSize,nOrients,clip,crop);
    float* H = new float[h*w*d];
    change_format_neon(H,HH,d,w,h);
